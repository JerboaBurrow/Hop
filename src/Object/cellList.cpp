...
void CellList::populate(ObjectManager * manager){
    ...
    std::unordered_map<std::string,Object>::iterator iter;
    int o = 0;
     std::unordered_map<std::string,Object> * objects = manager->getObjects();
    for (iter = objects->begin(); iter != objects->cend(); iter++){
        if (iter->second.isGhost()){continue;}
        uint64_t meshSize = iter->second.getCollisionMeshSize();
        for (int i = 0; i < meshSize; i++){
            const CollisionVertex p = iter->second.getCollisionVertex(i);
            // just incase!

...
namespace Hop::Object
{
    ...
    {
        std::vector<double> colour, transform, util, meshParameters;
        ...
        meshParameters = {CollisionPrimitive::RIGID, 1.0, 1.0};
        ...
        returnType = lua_getfield(lua, 1, "meshParameters");

        if (returnType == LUA_TTABLE)
        {
            meshParameters = getNumericLuaTable(lua,2);
            if (meshParameters.size() != 3)
            {
                lua_pushliteral(lua,"meshParameters requires 3 numbers");
                return lua_error(lua);
            }
        }
        else if (returnType == LUA_TNONE || returnType == LUA_TNIL)
